...
fmter = logging.Formatter(fmt="%(asctime)s.%(msecs)03d %(filename)s[line:%(lineno)d] %(levelname)s %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
    ...
    if not request.args or 'md5' not in request.args:
        ...
        for data in range(0, 80):
            ...
            if not result:
                ...
                break
        time_end = time.time()
        print("|#查询总耗时:%.2f秒|#MD5=%s|#UIN=%s" %
              ((time_end - time_start), md5, result[0]))
        return jsonify(result) if result else jsonify({'result': 'not found'})
